...
[![许可证](https://img.shields.io/github/license/Fahry-a/STA)](./LICENSE)
[![Cloudflare Workers](https://img.shields.io/badge/Cloudflare-Workers-orange?logo=cloudflare)](#self-deployment)

STA 是一个专为 Cloudflare Workers 优化的无服务器翻译服务。通过智能代理端点轮换、高级限流算法、缓存和熔断器机制，它比传统翻译 API 接入方式更能避免 HTTP 429 错误，同时带来更低的延迟。当前同时支持 DeepL 和 Google 翻译。

> [!NOTE]
> 与付费翻译 API 不同，STA 可以免费使用。无需 API 密钥、无需订阅，也没有按量计费。

## 为什么选择 STA

**与付费翻译 API 不同，STA 完全免费使用** - 无需 API 密钥、无订阅费用、无使用限制。只需部署一次，即可享受无限制的翻译请求，无需担心任何费用问题。
...
STA 在性能和稳定性方面相较于 DeepL API 有显著提升，以下是基于特定网络环境下的关键指标对比：

| 指标 | DeepL API | STA (预部署实例) |
|------|-----------|-------------------|
...
> **注意：** `PROXY_URLS` 视为机密，不写入 `wrangler.jsonc`。生产环境请用
> `wrangler secret put PROXY_URLS` 设置（本地 `wrangler dev` 用 `.dev.vars`）。
> 参见 `.dev.vars.example`。
...
1. 部署多个 XDPL 实例
2. 将部署后的 URL 添加到 STA 的 `PROXY_URLS` 机密（逗号分隔的列表）：
...
本地开发请用 `.dev.vars`（参见 `.dev.vars.example`）。
...
| 命令 | 说明 |
| --- | --- |
| `bun run dev` | 使用 Wrangler 启动本地开发环境 |
| `bun run deploy` | 部署到 Cloudflare Workers |
| `bun run cf-typegen` | 生成 Cloudflare Workers 类型定义 |
