...
| `APR` | boolean \| string | No | Array Per Request (default: `true`). Accepts a boolean, or the string sentinels `"true"`/`"false"` (case-insensitive, trimmed). |
...
The `apr` field mirrors the Array Per Request mode that was actually applied (`true` = one upstream call per item, `false` = one combined call). It is `false` on responses where validation never ran (e.g. a malformed-request 400), so the field is always a concrete boolean.
