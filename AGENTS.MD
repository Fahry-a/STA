...
- `bun run test:unit` - Run unit tests only (tests in tests/lib/)
- `bun run test:integration` - Run integration tests (tests/integration/)
- `bun run test:performance` - Run performance tests (tests/performance/)
- `bun run test:coverage` - Generate coverage report
- `bun run test:watch` - Run tests in watch mode
- `bun run test:debug` - Run tests with Node debugging enabled

### Code Quality
- `bun run lint` - Type-check with TypeScript (no emit, runs `tsc --noEmit`)
...
6. If cache miss:
   ...
   - Proxy health check (binary healthy/unhealthy with cooldown)
...
- **Proxy Manager** ([src/lib/proxyManager.ts](src/lib/proxyManager.ts)): Proxy selection with health tracking (binary healthy/unhealthy, 3-failure threshold, 30s cooldown)
...
The application exposes the following endpoints (defined in [src/index.ts](src/index.ts)):
- `POST /deepl` - DeepL translation (recommended)
- `POST /google` - Google Translate
- `POST /translate` - Legacy endpoint (uses DeepL for backward compatibility)
- `POST /v2/translate` - Batch translation with APR support
- `GET /health` - Health check
- `GET /metrics` - Performance metrics (auth required)
- `POST /admin/warm-cache` - Trigger cache warming (auth required)
- `GET /admin/cache-status` - Cache warming status (auth required)
- `POST /debug` - Debug endpoint (only available when `DEBUG_MODE=true`)
...
The worker includes a scheduled event handler that runs every 10 minutes (configured in `wrangler.jsonc` triggers):
- Clears in-memory cache to prevent memory leaks
- Warms cache with popular translations
- Called via `handleScheduled()` in [src/index.ts](src/index.ts)

## Key Implementation Details

### Rate Limiting Strategy
- **Client-level**: Token bucket with dynamic limit = `(proxy_count x 8 requests/sec) x 60 = tokens/minute`
...
### Caching Strategy
- **In-memory cache**: JavaScript Map (LRU) for fast lookups (cleared every 10 minutes by scheduled task)
- **KV cache**: Cloudflare KV for persistence (1-hour TTL)
- Cache key generation: SHA-256 hash of `text:source_lang:target_lang:provider`
- See [src/lib/cache.ts](src/lib/cache.ts)

### Proxy Health Tracking
- Binary healthy/unhealthy model (not a full circuit breaker with 3 states)
- Threshold: 3 consecutive failures marks proxy as unhealthy
- Cooldown: 30 seconds before retry
- Health resets after cooldown expires
- See `ProxyHealth` in [src/lib/proxyManager.ts](src/lib/proxyManager.ts)
...
- Language code validation with regex pattern
...
- Minimize memory allocations (cache cleanup every 10 minutes)
...
### Proxy Health Issues
- Proxies marked unhealthy after 3 consecutive failures
- 30-second cooldown before retry
- Check proxy health in proxy manager logs

### Cache Issues
- In-memory cache is cleared every 10 minutes by scheduled task
