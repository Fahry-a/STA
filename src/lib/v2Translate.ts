...
export async function translateBatch(
  ...
  if (APR) {
    return translateWithAPR(texts, source_lang, target_lang, env, clientIP, APR);
  }

  // APR=false: Combine texts and send as single request
  return translateCombined(
    texts,
    source_lang,
    target_lang,
    env,
...
async function translateWithAPR(
  ...
  return createV2Response(statusCode, resolvedResults, { apr });
}
...
async function translateCombined(
  ...
  try {
    ...
    if (result.code === 200 && result.data) {
      ...
      return createV2Response(200, results, { apr, id: result.id });
    } else {
      ...
      return createV2Response(result.code, results, { apr });
    }
  } catch (error) {
    const results: V2TranslationResult[] = texts.map((_, index) => ({
      text: "",
    ...
    return createV2Response(500, results, { apr });
  }
}
